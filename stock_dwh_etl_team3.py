...
AND EXISTS (
    ...
    WHERE AIRFLOW0105.DEV.DIM_COMPANY_PROFILE_3.COMPANY_ID = source.SYMBOL
    AND (
        AIRFLOW0105.DEV.DIM_COMPANY_PROFILE_3."NAME" IS DISTINCT FROM source.COMPANYNAME OR
        AIRFLOW0105.DEV.DIM_COMPANY_PROFILE_3.INDUSTRY IS DISTINCT FROM source.INDUSTRY OR
        AIRFLOW0105.DEV.DIM_COMPANY_PROFILE_3.SECTOR IS DISTINCT FROM source.SECTOR OR
        AIRFLOW0105.DEV.DIM_COMPANY_PROFILE_3.CEO IS DISTINCT FROM source.CEO OR
        AIRFLOW0105.DEV.DIM_COMPANY_PROFILE_3.WEBSITE IS DISTINCT FROM source.WEBSITE
    )
);
"""
...
WHEN NOT MATCHED THEN
    INSERT (COMPANY_ID, "NAME", INDUSTRY, WEBSITE, DESCRIPTION, CEO, SECTOR, 
            IS_CURRENT, EFFECTIVE_DATE, END_DATE)
    VALUES (source.COMPANY_ID, source."NAME", source.INDUSTRY, source.WEBSITE, 
